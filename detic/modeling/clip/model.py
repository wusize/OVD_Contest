...
class CLIP(nn.Module):
    ...
    def encode_text_with_prompt(self, text, prompt, normalize=True):
        x = self.token_embedding(text).type(self.dtype)  # [batch_size, n_ctx, d_model]
        prompt = prompt.type(self.dtype)

        x = torch.cat([x[:, :1], prompt, x[:, 1:]], dim=1)

        x = x + self.positional_embedding.type(self.dtype)[None, :x.shape[1]]
        x = x.permute(1, 0, 2)  # NLD -> LND
        x, _ = self.transformer(x, return_tokens=False, cls_indices=None)
        x = x.permute(1, 0, 2)  # LND -> NLD
        x = self.ln_final(x).type(self.dtype)

        # x.shape = [batch_size, n_ctx, transformer.width]
        # take features from the eot embedding (eot_token is the highest number in each sequence)
        out = x[torch.arange(x.shape[0]), text.argmax(dim=-1) + prompt.shape[1]] @ self.text_projection
        if normalize:
            out = F.normalize(out, p=2, dim=-1)
